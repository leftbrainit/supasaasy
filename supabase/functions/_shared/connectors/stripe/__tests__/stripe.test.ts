...
Deno.test('[Stripe] Extract Entity - customer deleted returns null', async () => {
  const mockEvent = createMockCustomerDeletedEvent({ id: 'cus_deleted' });
  const parsed = await stripeConnector.parseWebhookEvent(mockEvent, mockAppConfig);
  const entity = await stripeConnector.extractEntity(parsed, mockAppConfig);
...
Deno.test('[Stripe] Extract Entity - unknown resource returns null', async () => {
  const mockUnknownEvent = {
    id: 'evt_unknown',
    object: 'event',
    type: 'unknown.event.type',
...
// =============================================================================
// Extract Entities Tests (for subscriptions with items)
// =============================================================================

Deno.test('[Stripe] Extract Entities - subscription with items', async () => {
  const mockEvent = createMockSubscriptionEvent('customer.subscription.created', {
    id: 'sub_with_items',
    items: [
      { id: 'si_item1', priceId: 'price_1' },
      { id: 'si_item2', priceId: 'price_2' },
    ],
  });

  const parsed = await stripeConnector.parseWebhookEvent(mockEvent, mockAppConfig);

  // Use extractEntities if available
  if (stripeConnector.extractEntities) {
    const entities = await stripeConnector.extractEntities(parsed, mockAppConfig);

    // Should have subscription + 2 items = 3 entities
