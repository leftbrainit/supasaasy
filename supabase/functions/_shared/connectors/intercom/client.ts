...
export function createIntercomClient(appConfig: AppConfig): IntercomClient {
  ...
  return {
    ...
    async listCompanies(cursor?: string, perPage = DEFAULT_PAGE_SIZE) {
    ...
    async listContacts(cursor?: string, perPage = DEFAULT_PAGE_SIZE) {
      const params: Record<string, string> = {
        per_page: String(perPage),
      };
      if (cursor) {
    ...
    async listConversations(cursor?: string, perPage = DEFAULT_PAGE_SIZE) {
      const params: Record<string, string> = {
        per_page: String(perPage),
      };
      if (cursor) {
    ...
    async searchConversations(updatedSince: number, cursor?: string, perPage = DEFAULT_PAGE_SIZE) {
      const body = {
        query: {
          field: 'updated_at',
          operator: '>',
